...
| `d2_pickorder.py` | "Half its picks are settled before it looks at the board" — the exact pack-1-pick-1 ranking, the unconditional colour preference, the three-way attribution against human pick order / scorer value / encoder axes, and how much of the policy is context-free | `d2_pickorder.json`, `d2_p1p1_values.csv`, `d2_p1p1_logits.csv` |
| `d3_exchange.py` | "It picks differently, not better" — state exchange between agents, the two graders, and the per-pick policy-movement profile | `d3_*.json` / `.csv` |
...
| `d7_contextprobe.py` | "The trunk knows the seat's colours by pick 10" — ridge probes on the trunk summary token for the seat's eventual colours and its final pod-relative reward | `d7_contextprobe.json` |
...
| `d11_winratecolour.py` | "The colour preference comes from Forge's games" — the same colour pricing on the sealed self-play corpus the encoder and scorer were fitted to, at the card level and at the deck level, against build-method fixed effects and Forge's exogenous human draft rank; reads `Y:` and needs no GPU | `d11_winratecolour.json` |
