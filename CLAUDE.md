...
- `../forge` with `forge-connector/patches/0001-*.patch` applied (`git am` on a branch, so an upgrade rebases) — the engine hooks the collectors look up reflectively. Unpatched, collection still runs and stamps every record `degraded`.
