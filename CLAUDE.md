...
- **`analyze-generated-decks`** — the deck-composition diagnostic sourced from a `drafts.jsonl` corpus instead of a generated-decks file (`--drafts-path`, default `output/draft/drafts.jsonl`): each seat's built `deck` is analyzed by the shared `sealed` engine (colors, curve, types, lands, pips, rarity, with a per-label breakdown), and — because the corpus also carries each seat's `deck_score` — a per-label `deck_score` mean/median/n summary (the agent-vs-Forge headline) is printed above the composition report. Seats with a failed build (empty deck) are skipped.
