...
| FR-148 (new) | build-corpus drops records with a quality defect, for two reasons only: a resolution record with no acting ability, and any record with more than `--max-events-per-record` events (default 64); counts by reason in the manifest. An event attributed to `unresolved` is **counted, not refused** — the bracket collector records the events inside the ability's own resolution, so attribution to a clause can fail while the outcome is still the ability's. 12.3% of resolution records with an acting ability have every event stamped that way with no dumps among them, another 1% carry one unattributed side effect, and combat has no acting chain at all (98.9%, cause in `cause`); the dumps finding 2 measured are caught by the no-ability and event-flood rules. The manifest records the kept-record count as `unattributed_records`. The build stops rather than writing a dataset when the rules refuse more than half of any one sampling class |
...
| FR-078 | unchanged; add: the created-objects head's token-script id is the token's script stem (`c_a_food_sac`), falling back to the printed name only for a token whose script cannot be named — the collector's `SnapshotBuilder.entityToJson` writes it that way, and a shard collected before 2026-09-17 carries the printed name |
| FR-151 (new) | build-corpus remaps a provenance key whose script file the converted card tree does not hold and whose filename stem names a token script to `tokenscripts/<stem>.txt`, resolving the stem by name, then by the carrying entity's colours, types, supertypes and P/T, leaving a key untouched when more than one script remains — there is no trait-count step, because a snapshot shows a token's spell abilities only, so same-stat scripts differing by a keyword stay ambiguous. It corrects the token's own cast-spell key, which maps to no text either way; the ability lines were keyed correctly by the old collector. Records `token_keys_remapped`, `token_keys_ambiguous` (on written records) and `forge_tokenscripts` in the manifest; `--no-remap-token-keys` turns the step off |
