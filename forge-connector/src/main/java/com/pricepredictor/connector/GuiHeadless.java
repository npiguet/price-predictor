...
/**
 * Headless IGuiBase stub — satisfies the interface without any UI.
 * Adapted from the jumpstart-tierlist reference implementation.
 */
public class GuiHeadless implements IGuiBase {
    ...
    @Override public <T> List<T> getChoices(String message, int min, int max, Collection<T> choices, Collection<T> selected, FSerializableFunction<T, String> display) { return List.of(); }
    @Override public <T> List<T> order(String title, String top, int remainingObjectsMin, int remainingObjectsMax, List<T> sourceChoices, List<T> destChoices) { return List.of(); }
    ...
    @Override public String encodeSymbols(String str, boolean formatReminderText) { return ""; }
    @Override public void preventSystemSleep(boolean preventSleep) {}
    @Override public float getScreenScale() { return 0; }
