...
class TestSaveLoadRoundTrip:
    ...
    def test_saved_state_dict_excludes_regression_head(self, tmp_path: Path):
        cfg = _config()
        store = SealedEncoderStore()
        store.save_encoder(SealedEncoderModel(cfg), cfg, tmp_path, version="v2")
        payload = torch.load(tmp_path / "v2.pt", weights_only=False)
        keys = list(payload["model_state_dict"].keys())
        assert keys, "saved state dict must not be empty"
        assert all(
            k.startswith("token_encoder.") or k.startswith("card_encoder.")
            for k in keys
        )
        assert not any(k.startswith("regression_head.") for k in keys)
...
class TestLoadStrictness:
    def test_raises_on_extra_regression_head_key(self, tmp_path: Path):
        ...
        raw["model_state_dict"]["regression_head.0.weight"] = torch.zeros(1, 2 * cfg.d_model)
