...
class TestParsePools:
    def test_parses_single_pool(self, tmp_path):
        ...
        assert parse_pools(f) == [SealedPool("MH3", ["Card A", "Card B", "Card C"])]
    ...
    def test_parses_multiple_pools_with_different_set_codes(self, tmp_path):
        ...
        assert parse_pools(f) == [
            SealedPool("MH3", ["Flare of Denial", "Wight of the Reliquary"]),
            SealedPool("BLB", ["Moonrise Cleric", "Bark-Knuckle Boxer"]),
        ]
    ...
    def test_skips_blank_lines(self, tmp_path):
        ...
        assert len(result) == 2
        assert result[0].set_code == "MH3"
        assert result[1].set_code == "BLB"

    def test_pool_with_one_card(self, tmp_path):
        f = tmp_path / "pools.txt"
        f.write_text("MH3;OnlyCard\n", encoding="utf-8")
        assert parse_pools(f) == [SealedPool("MH3", ["OnlyCard"])]
