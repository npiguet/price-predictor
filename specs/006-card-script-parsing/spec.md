...
- What happens when an ability has no `SpellDescription$` or `TriggerDescription$`? The system should fall back to the Oracle text or mark the description as unavailable.
...
- **FR-001**: System MUST extract the following top-level properties from each card script: `Name`, `ManaCost`, `Types`, `PT`, `Loyalty`, `Colors`, `Text`, `Oracle`, `AlternateMode`.
...
- **FR-005**: System MUST extract activated abilities (`A:` lines with `AB$` or `SP$` prefix) and include only the cost (from `Cost$`) and the textual description (from `SpellDescription$`).
...
- **FR-009**: System MUST NOT include sub-abilities (`DB$` prefix lines defined in SVars) as separate ability entries, since they are part of their parent ability's description.
- **FR-010**: System MUST handle multi-face cards by splitting on the `ALTERNATE` delimiter and extracting each face as a separate card entry, linked by the `AlternateMode` value from the front face.
- **FR-011**: System MUST exclude engine-internal metadata from output, including `AI:`, `DeckHints:`, `DeckHas:`, `DeckNeeds:`, `SVar:`, and AI-specific parameters like `AILogic$`, `AITgts$`.
- **FR-012**: System MUST support batch processing of a directory tree of card scripts, producing one output entry per card face.
- **FR-013**: System MUST gracefully handle malformed card scripts by logging a warning with the file path and continuing to process remaining files.
- **FR-014**: System MUST substitute `CARDNAME` and `NICKNAME` placeholders with the actual card name in all extracted text descriptions.

### Key Entities

- **Card**: A single card face. Has top-level properties (name, mana cost, types, power/toughness, loyalty, colors, text, oracle text), a list of keywords, and a list of abilities. Multi-face cards produce multiple Card entities linked by alternate mode.
...
- **Ability**: A non-keyword ability on a card. Contains an optional cost and a textual description. Covers activated abilities, triggered abilities, static abilities, and replacement effects.
...
- The `Oracle` property on each card contains the complete official Oracle text as a fallback reference.
