...
4. **Given** Stage 2 training is running, **When** any step's total reward is computed, **Then** it falls within the
   range [-2, 2] — the sum of the Stage 1 budget signal (in {-1, +1}) and the discrete shaping term (in {-1, -0.5, 0, +0.5, +1}).
...
### User Story 2 — Discrete Imbalance Shaping Computation (Priority: P1)

The system needs to compute a discrete shaping signal at each step — measuring whether a pick improved or worsened
the deck's mana imbalance, with stronger signals when the imbalance is large. This computation is the mathematical
core of the per-step reward and must correctly track the moving ideal distribution, measure imbalance symmetrically,
and only activate once both pip demand and mana supply exist.

**Why this priority**: The shaping signal is the foundation of per-step credit assignment. If the signal is computed
incorrectly, the reward will send wrong signals to the model. Co-equal with Story 1 since neither works without
the other.

**Independent Test**: Can be tested independently by constructing known deck states and verifying that the discrete
shaping values (+1, +0.5, 0, -0.5, -1) match hand-calculated expected values.

**Acceptance Scenarios**:

1. **Given** no spells have been picked (pip demand empty) or no lands have been picked (actual sources empty),
   **When** the shaping signal is computed, **Then** it is 0.

2. **Given** imbalance before a pick is < 3.0 and the pick reduces imbalance, **When** the shaping signal is
   computed, **Then** it is +0.5.

3. **Given** imbalance before a pick is >= 3.0 and the pick reduces imbalance, **When** the shaping signal is
   computed, **Then** it is +1.0.

4. **Given** imbalance before a pick is < 3.0 and the pick increases imbalance, **When** the shaping signal is
   computed, **Then** it is -0.5.

5. **Given** a pick does not change imbalance, **When** the shaping signal is computed, **Then** it is 0.
...
### User Story 4 — ~~Configurable Hyperparameters~~ (REMOVED)

*Superseded by discrete shaping (2026-04-06). The discrete signal uses fixed thresholds (imbalance < 3 → half
magnitude, imbalance >= 3 → full magnitude) with no configurable hyperparameters. The `--urgency-exponent` and
`--temperature` CLI arguments have been removed.*

---

### Edge Cases

- What happens when no spells have been picked yet? Pip demand is empty → shaping = 0. The first spell pick
  activates pip demand but if no lands have been picked yet, actual sources is empty → shaping = 0. Shaping only
  activates once both pip demand and mana supply exist.
...
- What happens when a pick leaves the imbalance unchanged? Shaping = 0. The discrete signal only fires when
  imbalance strictly increases or decreases.
...
- Q: Which specific bounding function for the shaping signal? → A: Originally `tanh(x / temperature)`, replaced with discrete signal (see revision below).
- Q: Should recoverability ratio, imbalance, and shaping signal be logged as training metrics? → A: Log batch-mean shaping signal and batch-mean final imbalance on the existing print line.

### Revision 2026-04-06 — Discrete shaping

The continuous PBRS approach (`tanh(delta_ratio / temperature)`) produced shaping magnitudes ~0.001 for early picks
due to the `remaining^exponent` denominator, resulting in constant `shaping=-0.07` and no learning. Replaced with
a discrete signal: shaping = 0 when no pip demand or no mana supply; ±0.5 when imbalance < 3; ±1.0 when imbalance >= 3.
The `--urgency-exponent` and `--temperature` CLI arguments were removed.
...
- **FR-004**: System MUST compute the L1 imbalance before and after each pick to determine whether the pick
  improved or worsened the mana distribution.

- **FR-005**: System MUST compute the per-step shaping signal as a discrete value based on the imbalance change:
  shaping = 0 when pip demand is empty or actual sources are empty; ±0.5 when imbalance before the pick is < 3;
  ±1.0 when imbalance before the pick is >= 3. Positive when imbalance decreased, negative when increased, zero
  when unchanged. The signal is in the set {-1, -0.5, 0, +0.5, +1}.
...
- **FR-008**: ~~System MUST expose the urgency exponent and temperature as configurable hyperparameters.~~ REMOVED —
  the discrete shaping signal has no configurable hyperparameters.
...
- **Shaping Signal**: The per-step discrete reward component that measures whether a pick improved or worsened the
  deck's mana distribution. Values in {-1, -0.5, 0, +0.5, +1}, added to the Stage 1 budget reward.
...
- **SC-005**: Picks that worsen mana balance when imbalance >= 3 receive shaping of -1.0 (full magnitude), while
  picks that worsen balance when imbalance < 3 receive shaping of -0.5 (half magnitude), confirming the threshold
  amplification is working.
...
- The imbalance threshold of 3.0 and shaping magnitudes (0.5/1.0) are reasonable starting points. The threshold may
  need adjustment after observing training runs.
