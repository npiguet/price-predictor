...
- **FR-004**: The system MUST include common structural tokens in the vocabulary: punctuation marks, number values that appear in card text, and a placeholder token for card self-references (`CARDNAME`).
...
- **[UNK] Token**: The special unknown token produced when a word is not found in the vocabulary. Represents proper nouns, card-specific names, and any future terms not seen at vocabulary-building time. Does not carry word-specific information — the model treats all unknown words identically.
...
- Card names appearing in oracle text are normalized to a `CARDNAME` placeholder rather than added to the vocabulary, since there are 20,000+ unique card names and adding them would defeat the compact vocabulary goal.
