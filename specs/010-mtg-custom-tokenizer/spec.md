...
**Feature Branch**: `010-mtg-custom-tokenizer`
**Created**: 2026-02-28
**Status**: Draft
...
- **FR-001**: The system MUST build a domain-specific vocabulary from two sources combined: (a) hard-coded fixed lists of domain terms — game zone names, color names, multi-word keyword abilities (derived from `forge.game.keyword.Keyword` and stored as a constant `MULTI_WORD_KEYWORDS`); (b) corpus frequency scanning — all words appearing at or above a frequency threshold of 5 occurrences across the converted card corpus (`./output/`). No runtime I/O from Forge parser output files is required; domain term lists are constants in the application code. The combination ensures rare but valid domain terms are not missed.
...
- **FR-010**: The system MUST report vocabulary statistics after building: total token count, number of domain-specific tokens (from fixed domain lists), number of frequency-threshold tokens, corpus coverage percentage (fraction of token occurrences covered by vocabulary tokens, i.e., NOT mapped to `[UNK]`), and UNK rate (fraction mapped to `[UNK]` — equals 1 − coverage).
