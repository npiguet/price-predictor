...
| Card-disjoint validation splitting | `_split_cards` in `src/sealed/application/train_encoder.py:545` | **Reimplement, documented reason.** The discipline (card-level disjointness, never row-level) carries over, but neither the stratification key nor the exclusion rule does. Sealed splits by `score_play` quartile; the effects split takes ability texts by a stable hash of the text and holds out every card carrying one, and then **excludes from training every game holding a record that names a held-out card** — a whole-game exclusion sealed has no analogue for, because a sealed card row is not correlated with a board the way effect records within one game are. Sharing the function would mean parameterizing the two things that differ. |
...
- **Decision**: after choosing held-out texts (a stable hash of the text, over texts few cards
  carry) and the cards that carry them, **every game containing a record that names a held-out card is excluded from
