...
- [ ] T055 [P] Extend `tests/unit/draft/test_online_diagnostics.py` with the window-full guard: `observe_round` records no best while `window_drafts < anchor_window` (an early high margin cannot pin the run's best), records one on the first full-window round, and `best_margin`/`best_round` stay `None` for a run that never fills the window (data-model § 7, FR-033, SC-009)
- [ ] T056 [P] Stall-counter tests in `tests/unit/draft/test_online_run_control.py`: the counter starts at the first recorded best, increments once per round without a new best, resets on a new best **and** on each LR decay, and is the sole input to both knobs (data-model § 7, FR-035)
- [ ] T057 [P] Annealing tests in the same file: at `lr_decay_patience` the LR is multiplied by `lr_decay_factor`; no decay is taken that would land below `min_lr`; an armed `patience` never fires while a further decay is possible (SC-010); with `patience` unset the run anneals to the floor and continues; a decay does not re-run warmup or reset optimiser moments; a decay never reloads `best_*.pt` (FR-035)
- [ ] T058 [P] Validation tests in `tests/unit/draft/test_online_cli.py`: data-model § 1.1 rule 9 — each armed knob must exceed `ceil(anchor_window / drafts_per_round)`, `lr_decay_patience < patience` when both are armed, `0 < lr_decay_factor < 1`, `0 < min_lr <= lr` — each exiting `2` with a clear message before any update; and all four flags absent ⇒ byte-for-byte the pre-Phase-7 behaviour (FR-006, FR-034)
- [ ] T059 [US2] Extend `tests/integration/test_train_draft_agent_online_smoke.py`: a run whose window fills writes exactly one `best_*.pt` per new best and names it in the final summary; a run that ends before the window fills writes none and says so (FR-019, FR-033)

### Implementation

- [ ] T060 Add `patience`, `lr_decay_patience`, `lr_decay_factor`, `min_lr` to `TrainDraftAgentOnlineConfig` (data-model § 1) and rule 9 to `_validate_config` (§ 1.1), computing `window_rounds = ceil(anchor_window / drafts_per_round)` once and reusing it in both bounds
- [ ] T061 Add the window-full guard + `rounds_since_best` stall counter to `AnchorWindow` in `src/draft/application/train_draft_agent_online.py` (data-model § 7); `observe_round` returns whether this round set a new best so the loop can act on it
- [ ] T062 Wire `train_draft_agent._PlateauLR` + `_maybe_decay` into the online loop — **import, do not reimplement** (Principle VII; the gen-1 controller is already the third-instance home). Construct the controller at run setup, pass it to `_make_scheduler` as the existing `controller` argument so the decay multiplier folds into the same warmup lambda, and step the counter once per round
- [ ] T063 Write `best_{timestamp}.pt` on each new best via the unchanged `_write_checkpoint`, and add the `best …(round N, margin +X)` line to the incidental-log contract (contract § 3)
- [ ] T064 Add the `lr` field to `RoundDiagnostics` + the `movement` detail line, and the `LR decay #N -> …` log line (contract § 2/§ 3, data-model § 6)
- [ ] T064a [P] Cumulative-KL test in `tests/unit/draft/test_online_diagnostics.py`: `diagnostics_sweep` returns `kl_init_new` = 0 when all three models share weights, > 0 once the learner moves, unchanged when only `prev_model` moves (it must not read `prev_model`), equal to `kl_prev_new` when `π_0` and `π_k` coincide, and batch-size invariant (FR-016, research D17)
- [ ] T064b Add the third resident model to `TrainDraftAgentOnlineUseCase.execute`: an `init_model` loaded once from the **learner's** warm start (never the anchor — they differ whenever `--learner`/`--frozen` do), `eval()`, `requires_grad_(False)`, on the training device; extend `diagnostics_sweep` to forward it in the same batched `no_grad` pass and accumulate `KL(π_0 ‖ π_{k+1})` (data-model § 6, research D17). One extra forward per batch — no second pass
- [ ] T065 Implement the patience stop in `TrainDraftAgentOnlineUseCase.execute`: terminate exactly as `--max-rounds` does (final snapshot, final summary, exit `0`), only once no further decay is possible (FR-031(d), FR-034, SC-010)
- [ ] T066 Add `lr_decay_count` to `rl_metadata` as provenance and the `run ctrl` + window-geometry lines to the startup echo (`anchor window N drafts (R rounds, ~L-round lag)`) (contract § 1, data-model § 8)
- [ ] T067 Register `--patience`, `--lr-decay-patience`, `--lr-decay-factor`, `--min-lr` in `src/draft/infrastructure/cli.py`, all defaulting to disabled, mapping validation failures to exit `2` (contract § Flags)
- [ ] T068 Add the best-checkpoint path to `format_final_summary`, reporting "none recorded (anchor window never filled)" when there is no best (FR-019)
- [ ] T069 Update `README.md` + `CLAUDE.md` for the three artifact kinds and the opt-in run-control knobs; re-run the full gate (`pytest`, `ruff check src/ tests/`, `cd forge-connector && mvn test`)
