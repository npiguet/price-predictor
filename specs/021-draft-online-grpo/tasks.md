...
## Phase 7: Best checkpoint, patience & LR annealing (added 2026-08-05)

**Purpose**: FR-033…FR-035, added after the first gen-3 run peaked mid-run and
left no recoverable checkpoint at the peak, plus FR-016's cumulative KL (T064a/b),
added after a second run at `lr=1e-6` could not be diagnosed as
frozen-vs-creeping from the log alone. All three run-control knobs are opt-in, so
the default command surface and every Phase 1–6 test stay valid; the cumulative
KL is unconditional (one scalar from the existing sweep).

**Prerequisite**: Phases 3–4 complete (the loop, the anchor window, the
checkpoint cadence all exist).

### Tests (MANDATORY per Constitution) ✅

- [X] T055 [P] Extend `tests/unit/draft/test_online_diagnostics.py` with the window-full guard: `observe_round` records no best while `window_drafts < anchor_window` (an early high margin cannot pin the run's best), records one on the first full-window round, and `best_margin`/`best_round` stay `None` for a run that never fills the window (data-model § 7, FR-033, SC-009)
- [X] T056 [P] Stall-counter tests in `tests/unit/draft/test_online_run_control.py`: the counter starts at the first recorded best, increments once per round without a new best, resets on a new best **and** on each LR decay, and is the sole input to both knobs (data-model § 7, FR-035)
- [X] T057 [P] Annealing tests in the same file: at `lr_decay_patience` the LR is multiplied by `lr_decay_factor`; no decay is taken that would land below `min_lr`; an armed `patience` never fires while a further decay is possible (SC-010); with `patience` unset the run anneals to the floor and continues; a decay does not re-run warmup or reset optimiser moments; a decay never reloads `best_*.pt` (FR-035)
- [X] T058 [P] Validation tests in `tests/unit/draft/test_online_cli.py`: data-model § 1.1 rule 9 — each armed knob must exceed `ceil(anchor_window / drafts_per_round)`, `lr_decay_patience < patience` when both are armed, `0 < lr_decay_factor < 1`, `0 < min_lr <= lr` — each exiting `2` with a clear message before any update; and all four flags absent ⇒ byte-for-byte the pre-Phase-7 behaviour (FR-006, FR-034)
- [X] T059 [US2] Extend `tests/integration/test_train_draft_agent_online_smoke.py`: a run whose window fills writes exactly one `best_*.pt` per new best and names it in the final summary; a run that ends before the window fills writes none and says so (FR-019, FR-033)

### Implementation

- [X] T060 Add `patience`, `lr_decay_patience`, `lr_decay_factor`, `min_lr` to `TrainDraftAgentOnlineConfig` (data-model § 1) and rule 9 to `_validate_config` (§ 1.1), computing `window_rounds = ceil(anchor_window / drafts_per_round)` once and reusing it in both bounds
- [X] T061 Add the window-full guard + `rounds_since_best` stall counter to `AnchorWindow` in `src/draft/application/train_draft_agent_online.py` (data-model § 7); `observe_round` returns whether this round set a new best so the loop can act on it
- [X] T062 Wire `train_draft_agent._PlateauLR` + `_maybe_decay` into the online loop — **import, do not reimplement** (Principle VII; the gen-1 controller is already the third-instance home). Construct the controller at run setup, pass it to `_make_scheduler` as the existing `controller` argument so the decay multiplier folds into the same warmup lambda, and step the counter once per round
- [X] T063 Write `best_{timestamp}.pt` on each new best via the unchanged `_write_checkpoint`, and add the `best …(round N, margin +X)` line to the incidental-log contract (contract § 3)
- [X] T064 Add the `lr` field to `RoundDiagnostics` + the `movement` detail line, and the `LR decay #N -> …` log line (contract § 2/§ 3, data-model § 6)
- [X] T064a [P] Cumulative-KL test in `tests/unit/draft/test_online_diagnostics.py`: `diagnostics_sweep` returns `kl_init_new` = 0 when all three models share weights, > 0 once the learner moves, unchanged when only `prev_model` moves (it must not read `prev_model`), equal to `kl_prev_new` when `π_0` and `π_k` coincide, and batch-size invariant (FR-016, research D17)
- [X] T064b Add the third resident model to `TrainDraftAgentOnlineUseCase.execute`: an `init_model` loaded once from the **learner's** warm start (never the anchor — they differ whenever `--learner`/`--frozen` do), `eval()`, `requires_grad_(False)`, on the training device; extend `diagnostics_sweep` to forward it in the same batched `no_grad` pass and accumulate `KL(π_0 ‖ π_{k+1})` (data-model § 6, research D17). One extra forward per batch — no second pass
- [X] T065 Implement the patience stop in `TrainDraftAgentOnlineUseCase.execute`: terminate exactly as `--max-rounds` does (final snapshot, final summary, exit `0`), only once no further decay is possible (FR-031(d), FR-034, SC-010)
- [X] T066 Add `lr_decay_count` to `rl_metadata` as provenance and the `run ctrl` + window-geometry lines to the startup echo (`anchor window N drafts (R rounds, ~L-round lag)`) (contract § 1, data-model § 8)
- [X] T067 Register `--patience`, `--lr-decay-patience`, `--lr-decay-factor`, `--min-lr` in `src/draft/infrastructure/cli.py`, all defaulting to disabled, mapping validation failures to exit `2` (contract § Flags)
- [X] T068 Add the best-checkpoint path to `format_final_summary`, reporting "none recorded (anchor window never filled)" when there is no best (FR-019)
...
- [X] T070 [P] Registry tests in `tests/unit/draft/test_agent_registry.py`: `AgentRegistry.build` with an empty `agent_checkpoints` and every service supplied via `preloaded` validates labels and runs the geometry checks exactly as the path-loading route does, and the per-service `pick_mode` each was constructed with is preserved (the `build`-level `pick_mode` argument must not override a preloaded service) — the mechanism D5 relies on
- [X] T071 [P] Pick-mode wiring tests in `tests/unit/draft/test_online_cli.py` (or a new `test_online_pick_modes.py`): the use case builds the learner service at `pick_mode="sample"` with `temperature == rollout_temperature`, every `--frozen` service at `pick_mode="argmax"`, and passes all of them through `preloaded` with `agent_checkpoints` empty (FR-004, research D5)
- [X] T072 Change `TrainDraftAgentOnlineUseCase.execute` in `src/draft/application/train_draft_agent_online.py`: construct one `AgentPickService` per `--frozen` label at `pick_mode="argmax"` (temperature irrelevant there), merge with the learner's `sample`/`T` service, and pass the union as `preloaded` to `AgentRegistry.build` with `agent_checkpoints={}`. No `AgentRegistry` change is needed — `preloaded` (T038) already accepts arbitrarily-configured services
- [X] T073 Extend the startup echo's `rollout` line to state the split explicitly — e.g. `T=3.0 (learner samples; frozen play argmax)` — so a log alone shows which regime produced its margins (FR-013, contract § 1)
- [X] T074 Update the integration smoke test `tests/integration/test_train_draft_agent_online_smoke.py` for the new wiring, and document in `README.md` + `CLAUDE.md` that only the learner samples, that the margin therefore opens negative, and that margins do not compare across a change of `-T` / `--mix` / pick modes (FR-021a)
- [X] T075 Re-run the full gate (`pytest`, `ruff check src/ tests/`); no Java change is involved
...
- [X] T076 [P] Parser tests in `tests/unit/draft/test_online_cli.py`: `"gen3=3,gen1=2"` → `{"gen3": 3.0, "gen1": 2.0}`; whitespace tolerated; a single entry works; duplicate label, empty label, empty/non-numeric value, and a trailing comma each exit 2 with a clear message
- [X] T077 [P] Validation tests in `tests/unit/draft/test_online_cli.py` (FR-024 rule 5, exit 2 each): `--agent-temp` omitted entirely; the map omits the learner label; the learner's value is `0` or negative; the map names a Forge built-in (`forge-full`); the map names a label that is neither the learner nor a `--frozen` label; any negative value. Each must fail **before** the worker launches, alongside the existing rules
- [X] T078 [P] Pick-mode wiring tests in `tests/unit/draft/test_online_pick_modes.py`: extend the Phase 8 fixture so an unnamed `--frozen` label still yields `_pick_mode == "argmax"` (the Phase 8 property, now the default) while a `--frozen` label named in the map yields `_pick_mode == "sample"` at its own temperature; the learner is always `sample` at its map value
- [X] T079 Replace the flag in `src/draft/infrastructure/cli.py`: drop `-T` / `--rollout-temperature`, add `--agent-temp` (required) with the comma-separated `LABEL=VALUE` parser. Reuse `parse_agent_checkpoints`'s `LABEL=…` splitting conventions and error style rather than inventing a second dialect (Principle VII)
- [X] T080 Change `TrainDraftAgentOnlineConfig` in `src/draft/application/train_draft_agent_online.py`: `agent_temperatures: dict[str, float]` becomes the stored field; `rollout_temperature` becomes a derived read of `agent_temperatures[learner_label]` so every existing loss/diagnostic use site keeps working unchanged. Add FR-024 rule 5 to the startup validation
- [X] T081 Build each service from its map entry in `execute`: `pick_mode="argmax"` when the label is absent, `pick_mode="sample"` at that `T` when present; the learner always `sample`. Still every service through `preloaded` with `agent_checkpoints={}` (Phase 8's mechanism is unchanged)
- [X] T082 Extend the startup echo's `rollout` line to print the resolved map — every label with its temperature or `argmax` — so a log alone identifies the field that produced its margins (FR-013, contract § 1)
- [X] T083 Write `agent_temperatures` into `rl_metadata` alongside the existing `rollout_temperature` (FR-027), so a checkpoint records the field it was trained against; `rollout_temperature` keeps its current meaning (the learner's own) for back-compatibility with gen-2-era readers
- [X] T084 Update the integration smoke test `tests/integration/test_train_draft_agent_online_smoke.py` for the new flag, and re-run the full gate (`pytest`, `ruff check src/ tests/`); no Java change is involved
