...
5. **Given** an invalid startup configuration (missing base checkpoint, architecture/embedding-width mismatch, missing learner agent, or an `--agent-temp` map that is missing, omits the learner, gives the learner a non-positive value, or names a Forge built-in or unknown label), **When** the operator starts training, **Then** it exits nonzero with a clear message before any update.
...
- **Run ends before the anchor window fills**: no best margin was ever well-defined, so no best checkpoint exists → the final summary reports that plainly rather than naming a checkpoint chosen on a partly-filled window (FR-033).
- **Annealing reaches the LR floor**: no further decay is taken. With `--patience` armed the run then stops at the next stall; without it, the run continues at the floor until interrupted (FR-035).
- **`--patience` or `--lr-decay-patience` shorter than the anchor window (in rounds)**: rejected at startup — the margin could not yet reflect an improvement, so the knob would act on noise (FR-034, FR-035).
...
- **FR-004**: The command MUST require a per-label temperature map `--agent-temp "LABEL=T,…"` (no default), and MUST NOT expose a single global rollout-temperature flag. A label the map omits plays **argmax** (equivalently `T = 0`); a label it names with `T > 0` is sampled at that `T`. The map MUST name the `--learner` with a positive value: sampling is the learner's sole exploration mechanism, and the learner's own `T` is also the temperature every policy distribution used in the update (log-probs, entropy, KL) MUST be evaluated at, which keeps the update on-policy by construction. Every **frozen** agent plays its argmax — its best — **unless the operator names it in the map**, which is a deliberate experiment rather than a default. Sampling a frozen agent would not merely lower its own score: drafting allocates a fixed pool of cards, so a card it wrongly passes flows downstream and is received by its podmates, the learner among them. A sampled field therefore feeds the learner cards a properly-playing agent would have kept, making the training environment easy in exactly the dimension drafting is about (what wheels, and what remains when the pack returns) and unlike the evaluation field of FR-030, where every agent plays argmax. Naming a `--frozen` label is therefore permitted but changes the field, resetting the margin's zero point (FR-021a) and making margins incomparable across the change. Forge built-ins MUST NOT appear in the map — their randomisation is internal to Forge and no temperature applies — and naming one is a startup error (FR-024).
...
- **FR-006**: The command MUST expose exactly three knobs the operator is expected to tune for *learning* — learning rate, the `--agent-temp` map, and drafts-per-round — and MUST NOT expose a critic weight, GAE-λ, KL coefficient, entropy coefficient, or any **loss-coefficient** decay schedule (no loss-term coefficients exist to schedule). Every other learning setting (batch size, gradient-clip norm, warmup length, one pass per round, the learner label) carries a fixed default the operator is not expected to touch. Run-control knobs that govern when a run stops or anneals rather than how it learns — `--patience` (FR-034) and the LR-decay flags (FR-035) — are a separate, opt-in category and are all disabled by default, so the default command surface is unchanged.
...
- **FR-013**: At startup the system MUST echo the resolved run configuration and the results of the FR-024 validation to stdout: learner label + warm-start checkpoint, generation index, frozen/anchor labels, scorer checkpoint + build method (+ picker checkpoint when picker), the resolved per-label temperature map (each label with its temperature, or `argmax`), learning rate, drafts-per-round, batch size, the random seed and the FR-032 note that Forge-side rollout randomness is unseeded, device, and the embedding-cache/checkpoint width. (Mirrors the gen-2 trainer's startup echo.)
...
- **FR-016**: Each round MUST log a **movement** line: the mean log-probability of taken actions, the mean policy-gradient loss term for the round, and **two** KLs (both logged, unpenalised) — the KL to the *previous round's* policy `KL(π_k ‖ π_{k+1})`, and the KL to the run's *warm-start* policy `KL(π_0 ‖ π_{k+1})`. A large or erratic per-round KL signals the step was too large for the round size; a cumulative KL that stays flat signals the step is too small to move the policy at all. Both are required because neither implies the other: small steps taken in a consistent direction accumulate, so a near-zero per-round KL does **not** establish that the policy is stationary, and only distance from `π_0` shows it. `π_0` is the learner's own warm-start checkpoint (FR-002), never the anchor — they coincide only when `--learner` and `--frozen` name the same file, which the command does not require.
- **FR-017**: Each round MUST compute and log the **anchor margin** live — mean deck-quality of the learner seats minus mean deck-quality of the anchor's seats, over a sliding recent-drafts window — alongside the raw component means (the learner label, the anchor label, and every Forge label in the mix) and the window size / draft count backing them. This is the round-over-round absolute-progress signal and MUST NOT require pausing training or a separate command to read.
...
- **FR-019**: At run end or on operator interrupt the system MUST log a **final summary**: rounds completed, total drafts generated, the latest checkpoint path, the best-checkpoint path (FR-033, or an explicit "none" when no best was recorded), and the best anchor margin observed and the round it occurred at.
...
- **FR-021**: The frozen reference used for the anchor margin MUST stay fixed for the whole run (never swapped to a later generation or to "previous round") and MUST play at argmax (FR-004), so that what it passes downstream is the play of a properly-functioning agent rather than a deliberately noised one.
- **FR-021a**: Two properties of the anchor margin follow from seats competing for one pool of cards, and MUST be documented rather than corrected for. (a) **The margin does not start at zero.** At round 0 the learner carries its sampling handicap while the anchor carries whatever `--agent-temp` gives it — nothing, by default — so the margin opens negative by the *difference* between the two handicaps; round 0 measures that offset directly, since learner and anchor start from identical weights. Under the default argmax anchor the offset is the learner's own handicap and crossing zero means it has overtaken a properly-playing anchor; if the anchor is given a temperature the offset shrinks and zero loses that meaning, so the round-0 reading — not zero — is what defines the scale. (b) **The margin is field-relative, and MUST NOT be discounted.** A learner that improves also takes cards its podmates would have had, so the learner rises *and* the anchor falls. That is the game, not measurement inflation: an 8-seat pod allocates one shared pool, denial is a legitimate draft strategy, and the FR-009 reward is itself pod-relative (leave-one-out `deck_score`), so the objective pays for it directly. There is no smaller "standalone" figure underneath — the margin is the competitive result against a **stated field**, which MUST be reported alongside it. The FR-030 yardstick co-seats on the same principle and is read the same way. Margins are consequently **not comparable** across a change of field — `--mix`, `-T`, or the FR-004 pick modes.
...
- **FR-024**: At startup the system MUST validate that the learner's warm-start checkpoint exists and its architecture matches the embedding-cache width, that exactly one learner agent is specified, that the required scorer checkpoint exists (and the picker checkpoint exists when the build method is picker), that the `--agent-temp` map is supplied, names the learner with a positive value, names no Forge built-in and no label outside `--learner` / `--frozen`, and carries no negative value, and that the agent-label wiring is consistent (FR-003: every mix label is a known built-in / frozen / learner label; the learner label is in the mix and unbound; the anchor is a frozen label in the mix, or defaults to the sole frozen agent); failures exit nonzero with a clear message.

**Optimisation & checkpoints**

- **FR-025**: Optimisation MUST reuse the gen-1/gen-2 trainer conventions (adaptive optimiser with per-group gradient-norm clipping, linear-warmup-then-constant learning rate, fixed batch size within the 8 GB VRAM budget). The optimiser state and learning-rate schedule MUST be continuous across rounds (a single warmup at the start of the run, then constant — subject to the optional plateau annealing of FR-035), not reset per round. The per-round gradient norm (pre-clip) and the current learning rate MUST be included in the round's movement diagnostics (FR-016).
- **FR-026**: The online loop MUST NOT use a **held-out-loss** best-checkpoint / early-stop guard — no held-out split exists. It MUST persist a "latest" checkpoint each round plus periodic timestamped snapshots. Checkpoint *selection* is assisted by the in-loop anchor margin (FR-033) but never settled by it: promotion remains the operator's judgment, driven by the live margin (FR-017) and the external yardstick (FR-030). Stopping is likewise the operator's, except where they explicitly arm FR-034.
- **FR-027**: The next checkpoint MUST be written in the gen-1 agent checkpoint format (trunk + policy + critic + recency/context tables, config, round/epoch counters) plus gen-3 RL metadata: the generation index, the base-checkpoint identity, the algorithm tag (`online-grpo`), the RL hyper-parameters (learning rate, the learner's own rollout temperature, the whole resolved `--agent-temp` map so the field is reconstructable, drafts-per-round), and the LR-decay count reached (FR-035) as provenance — recording how far a run annealed. The decay count MUST NOT be restored on a later run: gen-3 has no `--resume`, and restarting from a snapshot deliberately re-runs warmup and resets both the optimiser moments and the decay position. The critic head is carried unchanged; no critic, GAE, KL, or entropy hyper-parameters are stored. Encoder weights are not trained or stored (Phase A).
...
**Run termination & determinism**

- **FR-031**: A run MUST terminate in exactly one of four ways, each of which first writes the final checkpoint and prints the FR-019 final summary: (a) an operator interrupt; (b) an optional operator-supplied round budget being reached; (c) the inherited pick-fault abort after N consecutive abandoned drafts, which MUST exit nonzero; or (d) the opt-in stalled-margin stop of FR-034, when the operator has armed it. Only (d) is quality-driven, it is off by default, and it is driven by the in-loop anchor margin rather than a held-out loss (which does not exist — FR-026).
- **FR-032**: The command MUST accept a random seed that fixes everything it owns — model/optimiser initialisation, per-round batch shuffling, and the pick-sampling RNG of every seat that samples (the learner's; frozen agents play argmax and draw no randomness — FR-004) — and MUST state at startup that the rollout randomness Forge owns (booster contents, per-draft set choice, the Forge AI, the per-seat mix draw) is not seeded, so a run reproduces only up to the rollouts it is given.

**Best-checkpoint selection, patience & LR annealing**

The anchor margin (FR-017) is the only in-loop quality signal, so it is what
selects a best checkpoint and what the optional run-control knobs key off. Its
sampling properties are therefore normative, not incidental: the margin is
averaged over `--anchor-window` drafts, which spans
`--anchor-window / --drafts-per-round` **rounds**, and it therefore trails the
current policy by roughly half that span.

- **FR-033**: The system MUST track the best anchor margin observed and write a distinct best checkpoint (`best_{timestamp}.pt`, alongside `latest.pt` and the periodic snapshots) each time a new best is reached. Two rules keep the selection honest: (a) **window-full guard** — no best MAY be recorded until the sliding window holds a full `--anchor-window` drafts, so an early round computed over a partly-filled window cannot pin the best for the whole run; a run that ends before the window fills MUST report no best rather than a spurious one; and (b) the recorded best is a maximum over a correlated series and is therefore optimistic — it selects a checkpoint and MUST NOT be presented as an unbiased measurement of that checkpoint's strength (the yardstick, FR-030, remains the measurement). Because of the margin's lag, periodic snapshots (FR-026) MUST continue to be written even when a best checkpoint exists: the genuinely best policy may precede the recorded best by a few rounds, and a snapshot is how it stays recoverable.
- **FR-034**: The command MUST accept an optional `--patience N` that stops the run after `N` consecutive rounds with no new best anchor margin, terminating exactly as the other modes do (FR-031: final checkpoint, final summary). It MUST be disabled by default, so the default contract remains "no automatic stop". `N` MUST be greater than the window length in rounds (`--anchor-window / --drafts-per-round`); otherwise the command MUST fail fast at startup (FR-024), because a genuine improvement needs a full window to propagate into the margin and a shorter patience would stop before the evidence arrives.
- **FR-035**: The command MUST accept optional plateau LR annealing, mirroring the gen-1 trainer's convention with rounds as the unit: `--lr-decay-patience N` (disabled by default), `--lr-decay-factor` (default 0.1) and `--min-lr` (default `lr × 1e-3`). Patience (FR-034) and annealing MUST share one **stall counter** — rounds since the last new best anchor margin — which starts once the first best is recorded, resets on every new best, and resets again on each decay. When the counter reaches `--lr-decay-patience` and a further decay would stay at or above `--min-lr`, the learning rate MUST be multiplied by `--lr-decay-factor`. Consequences that MUST hold: (a) because each decay resets the counter, an armed `--patience` cannot fire until the LR floor is reached, so an unattended run anneals before it gives up; (b) `--lr-decay-patience` MUST satisfy `anchor_window / drafts_per_round < --lr-decay-patience < --patience` when both are armed, and the lower bound alone when only annealing is, else fail fast; (c) annealing MAY be armed with stopping disabled, in which case the run anneals to the floor and continues until interrupted; (d) a decay multiplies the post-warmup constant LR — warmup MUST NOT be re-run and optimiser moments MUST NOT be reset; and (e) a decay MUST continue from the current weights and MUST NOT reload the best checkpoint, since the margin's best is an optimistic maximum (FR-033) and returning to it repeatedly would chase selection noise. Each decay MUST be logged as it happens.
...
SC-001, SC-002, and SC-005 are **campaign-time** criteria: they are judged from a
yardstick run plus the accumulated run log once the feature ships, and their
outcome is recorded in `experiments/` — no automated test verifies them. The
run-to-run noise band SC-001 compares against is the one the yardstick itself
measures (a repeat of the same fixed-mix run). The remaining criteria are
verifiable from the build.
...
- **SC-003**: Every round that takes an optimizer step emits to stdout a reward-signal line (reward mean/std + advantage spread and near-zero fraction), an exploration line (entropy / perplexity / off-argmax rate), a movement line (mean logπ, policy-loss term, gradient norm, KL-to-previous-round), a live anchor-margin line (margin + raw component means + window size), and a consolidated round-summary line — so the four diagnostic axes are all present in the log for every such round. A degenerate round (FR-023) emits its round-summary line marked as skipped-with-no-signal, carrying the anchor margin, in place of the four detail lines — so no round is ever silent.
...
- **SC-006**: 100% of runs with an invalid startup configuration (missing/architecture-mismatched base checkpoint, missing learner agent, or an `--agent-temp` map that is missing, omits the learner, gives the learner a non-positive value, or names a Forge built-in or unknown label) fail fast before any update, with a clear message.
...
- **SC-009**: A best checkpoint exists on disk for every run whose anchor window filled, and for no run whose window did not; in 100% of runs no best is recorded before the window is full, so a partly-filled early round can never pin the run's best.
- **SC-010**: With annealing armed, a stalled run anneals rather than stops: an armed `--patience` never terminates a run whose learning rate is still above `--min-lr`.
...
- The gen-2 offline recipe (multi-epoch reuse of a frozen corpus, learned critic + GAE, KL anchor, entropy bonus, and any val-keyed **loss-coefficient** decay schedule — those coefficients do not exist here). The *learning-rate* schedule is a separate matter and is retained: warmup, plus the optional plateau annealing of FR-035.
...
- The Bradley-Terry "beat the pod" objective — the reward stays the pod-relative deck-quality score.
