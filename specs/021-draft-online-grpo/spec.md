...
- **FR-032**: The command MUST accept a random seed that fixes everything it owns — model/optimiser initialisation, per-round batch shuffling, and the pick-sampling RNG of every model-piloted seat — and MUST state at startup that the rollout randomness Forge owns (booster contents, per-draft set choice, the Forge AI, the per-seat mix draw) is not seeded, so a run reproduces only up to the rollouts it is given.
