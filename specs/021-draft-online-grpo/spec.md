...
- **FR-006**: The command MUST expose exactly three knobs the operator is expected to tune — learning rate, temperature `T`, and drafts-per-round — and MUST NOT expose a critic weight, GAE-λ, KL coefficient, entropy coefficient, or any coefficient-decay schedule. Every other setting (batch size, gradient-clip norm, warmup length, one pass per round, the learner label) carries a fixed default the operator is not expected to touch.
...
- **FR-031**: A run MUST terminate in exactly one of three ways, each of which first writes the final checkpoint and prints the FR-019 final summary: (a) an operator interrupt; (b) an optional operator-supplied round budget being reached; or (c) the inherited pick-fault abort after N consecutive abandoned drafts, which MUST exit nonzero. None of the three is a *quality*-driven stop — there is still no held-out-loss guard, best-checkpoint selection, or early stop (FR-026).
