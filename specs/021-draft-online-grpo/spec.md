...
- **FR-004**: The command MUST require the rollout temperature `T` (positive, no default); all policy distributions used in the update (log-probs, entropy) MUST be evaluated at that same `T`. `T` governs the **learner's** seats only: the learner is sampled at `T` because sampling is its sole exploration mechanism. Every **frozen** agent MUST instead play its argmax — its best. Sampling a frozen agent would not merely lower its own score: drafting allocates a fixed pool of cards, so a card it wrongly passes flows downstream and is received by its podmates, the learner among them. A sampled field therefore feeds the learner cards a properly-playing agent would have kept, making the training environment easy in exactly the dimension drafting is about (what wheels, and what remains when the pack returns) and unlike the evaluation field of FR-030, where every agent plays argmax. Forge built-ins are unaffected: their randomisation is internal to Forge and independent of `T`.
