...
- **FR-021a**: Two properties of the anchor margin follow from seats competing for one pool of cards, and MUST be documented rather than corrected for. (a) **The margin does not start at zero.** At round 0 the learner carries its sampling handicap while the anchor carries none and passes nothing away, so the margin opens negative; round 0 measures that offset directly, since learner and anchor start from identical weights. (b) **The margin amplifies.** A learner that improves also takes cards its podmates would have had, so the learner rises *and* the anchor falls, and the measured margin moves at roughly twice the underlying skill gap. Both properties are shared by the FR-030 yardstick, which also co-seats the agents being compared — so the two instruments stay consistent, but neither is an absolute measure of how much better one agent is. Margins are consequently **not comparable** across a change of `-T`, of `--mix`, or of the FR-004 pick modes.
