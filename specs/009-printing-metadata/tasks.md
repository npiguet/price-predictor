...
- [ ] T010 Update `parse_converted_text` in `src/price_predictor/infrastructure/converted_card_parser.py` to recognize the 5 new key:value fields (`reserved:`, `rarity:`, `printings:`, `set:`, `legalities:`) and populate `Card.printing_data` with a `PrintingData` instance when any are present. Fields not present in the text leave `printing_data` as `None` (all-or-nothing) or partially filled via a builder approach — decide based on simplicity.
- [ ] T011 [P] Add unit tests for enriched card text parsing in `tests/unit/infrastructure/test_converted_card_parser.py` — test parsing card text with all 5 metadata fields, with no metadata fields (backward compat), with partial metadata, and with empty legalities
- [ ] T012 Create `src/price_predictor/application/card_enrichment.py` with: (a) `enrich_card_text(text: str, printing_data: PrintingData) -> str` that appends the 5 metadata lines at the end of card text; (b) `extract_printing_fields_from_text(text: str) -> dict[str, str]` that detects which metadata keys are already present in the text and returns their values; (c) `enrich_or_default(text: str, metadata_map: dict[str, PrintingData]) -> str` that parses the card name from text, looks it up in the metadata map, merges client-provided fields with auto-filled or default values, and returns enriched text
...
- [ ] T019 [US1] Update `TrainModelUseCase.execute` in `src/price_predictor/application/train.py` — after joining cards to prices, call `build_metadata_map` and attach `PrintingData` to each training Card. The Card entity is frozen, so create a new Card instance with `printing_data` set.
