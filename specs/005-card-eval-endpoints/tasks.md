...
- [x] T009 Update README.md with eval subcommand documentation — add `eval` to the CLI commands section, document usage syntax (`python -m price_predictor eval <file> [--endpoint URL]`), describe structured request logging behavior and log format for the serve command.
- [x] T010 Run quickstart.md validation — verify all commands in specs/005-card-eval-endpoints/quickstart.md work as documented: serve starts, curl returns response, eval subcommand works, error cases produce expected messages.
