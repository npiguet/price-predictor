...
| VI. Documentation | **PASS** | `contracts/cli.md` documents both new subcommands and every flag, exit code, and error condition. `contracts/checkpoint-format.md` documents the `.pt` payload schema and resume-precedence rule. `quickstart.md` is an end-to-end walkthrough from setup to self-play participation. Both `CLAUDE.md` (T032, T033) and the root `README.md` (T037) are updated with `train-picker` / `pick-decks` entries during implementation — the README documents each sealed subcommand in its own section, so the constitution VI "README explains how to launch all workflows" requirement is satisfied in the same PR. |
