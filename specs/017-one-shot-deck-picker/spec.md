...
- **FR-019**: At the end of every epoch, the system MUST run deterministic inference (FR-006) on the entire validation slice, score the resulting chosen-card decks (the FR-006 output — spells + nonbasic lands, before the FR-007 basic-land fill) with the frozen training scorer, and report mean reward. Best-checkpoint selection MUST use this metric.
