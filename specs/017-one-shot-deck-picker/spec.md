...
- **Empty or near-empty pool.** A pool with fewer than 23 nonland cards cannot satisfy the spell quota. The pick-decomposition walk must terminate gracefully (take what spells exist, fill the rest with basic lands) and `pick-decks` must produce a 40-card deck for every input pool regardless. This is an upstream-data hygiene issue; the picker does not enforce pool minimums but does not crash on them either.
- **Pool with more than 23 nonbasic lands ranked above all spells.** The spell-quota loop unconditionally requires 23 spells, so the picker walks deep into the sorted logits if necessary. The total picks may exceed 40 - 17 = 23 + nonbasic-lands picked; the basic-land fill is `40 - len(chosen)`, which can legitimately be zero (if the picker selected 40 cards' worth of lands and spells through the walk, no basic lands are added). The walk halts at the spell quota, so this scenario terminates cleanly.
...
- **FR-011**: For each training step the loop MUST: draw a batch of pools, run one picker forward per pool, sample multiple decks per pool from the picker's distribution, score every sampled deck with the frozen scorer in one batched forward, compute the per-pool baseline and advantage, compute the policy-gradient + entropy + auxiliary loss, and step AdamW on the picker parameters only. The scorer and the encoder used to produce the `.npz` cache MUST remain frozen throughout training.
- **FR-012**: Deck sampling MUST be sequential categorical sampling without replacement over the full pool under softmax-of-logits at the configured temperature, halting when the 23-spell quota is met. Each sampled "deck" passed to the scorer is the chosen spells + nonbasic lands plus the basic-land fill.
