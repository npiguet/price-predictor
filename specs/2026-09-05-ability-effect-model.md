...
- **A bus subscription**, where Forge already broadcasts the fact on its own game-event bus and a collector listens. Six types arrive this way now — `energy_change`, `radiation_change`, `speed_changed`, `day_night_changed` from the original plan, plus `regenerated` and `library_shuffled`, wired by a Task 10 fix round once a review found the engine was already broadcasting both into a void (`GameEventCardRegenerated`, `GameEventShuffle`) and neither needed a schema change — subjects-only types satisfy a bus subscription with nothing more. `energy_change` is the channel's canary: Aether Hub is the cheapest sealed-legal card that exercises it.
...
- `test_no_declared_type_is_unreachable_by_surprise` (`test_event_schema_completeness.py`) statically scans the connector's Java source for a reference to each type's constant, and fails if the set of types with no reference at all (`KNOWN_UNEMITTED`) changes without the test being updated to match. **It checks against the full 78-member declared vocabulary; until this fix round it checked against only the 57 types with their own params row, so the other 21 were never compared at all** — the same "scans references, not fires" failure mode, recurring one level up, inside the guard. Widening it surfaced 15 genuinely unreferenced types (`KNOWN_UNEMITTED` is no longer empty, and that is the check finally telling the truth, not a regression); even at full width it still **cannot** catch a type referenced from code that never runs on the expected path. `damage_prevented` and `spell_copied` are the original pair, referenced in dead code. A Task 10 measurement found four more shapes of the same blind spot, in two kinds: `spell_countered` and `token_created` are referenced by a live **reactive-trigger** mode name — a card reacting to the outcome, never the resolving ability's own announcement — while `dice_rolled` and `player_won` are referenced by a live **replacement** mode name instead, which fires only when a replacement effect intercepts the roll or the win in progress, not a trigger and not the ability's own resolution either. Two more candidates in this category, `regenerated` and `library_shuffled`, turned out on closer reading to need different fixes — one had a genuinely dead table entry, the other a live-but-differently-purposed one — and both are now wired via the bus mechanism above instead of documented here.
...
The itemized rulings behind the less obvious corners of the vocabulary — `coin_flipped`'s two result dialects and its `FlipUntilYouLose` loss count, `damage_prevented`'s relationship to its `rewrite` sibling, `restriction_change`'s subject convention, `vote_taken`'s ballot, the two `choice_made`-family keys that are not spelled like their effect classes, Clash's reveal, the simulated-game guard, the 15 types the guard's own domain hid, and the four declared types found not firing and since wired by a Task 11 fix round (`spell_countered`, `token_created` and `dice_rolled` via `EffectRecordOutcomes.note`, `player_won` off the bus) — in the two distinct sub-shapes above, which explain why the static guard's reference to each could not be trusted, not why they are still missing — are recorded in `023-ability-effect-model/contracts/record-schema.md` § Events, alongside the field-level contract they each constrain.
...
- Variant cards are decked and scheduled as coverage decks are (§ Coverage collector), over the variant set rather than the converted corpus, with the same `--decks-per-round`. One invocation plays one round: coverage re-weights each round by how far a card is from its target, while every variant is weighted equally, so there is no shortfall to converge on. `--seed` (default 42) seeds both the perturbation and the deck build, and a repeat run needs it changed or it regenerates the same scripts and decks.
...
- **Validation.** Captured once from the reserved shards and reused every epoch, sampled to the **training** mixture rather than to the proportions the stratum holds on disk, and drawn from shards taken across the stratum rather than off the front of its list. Scored in `--batch-size` batches over the shuffled sample: a batch's class composition decides which fields carry loss, so a whole-game batch measures a different objective than the training batch printed beside it.
- **Progress.** Each shard's line carries its mean training loss, the loss decomposed by field, each module's pre-clip gradient norm, the learning rate and the step rate. One step per shard — the last — reads those back, since each read is a device synchronization. The three-way training / card-disjoint / game-disjoint comparison stays at the close of each epoch, with the card-disjoint loss broken down by field.
