...
The response sizes are what is wrong: an edit that changes nothing moves the prediction about as much as an edit that guts the card. Rewriting a Pacifism's line from "enchanted creature can't attack or block" to "CARDNAME can't attack or block" turns the card into a no-op, because the enchantment itself could never attack or block anyway. The prediction does drop, but only by −0.12 SD, with a third of the edited auras moving the wrong way; swapping the same card's two static lines, which changes nothing at all, moves it three times as much. Every context response points the right way; against the noise from meaningless edits, none stands out the way a real difference in card quality should.
