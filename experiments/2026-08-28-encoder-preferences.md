...
Creatures earn most of their premium through the selection channel. Creature-heavy decks are the decks that win in this corpus. The label shares each win's credit among every card the winner cast, so a creature in a strong deck collects credit whether or not it caused the win. Creatures are also cast far more often than noncreatures, and that difference earns them almost nothing. The rest of the premium is contribution: winning game states have creatures on the battlefield. Unconditional removal has the same composition, with most of its premium in the selection channel.

A combat trick reaches nearly the same premium as a creature with essentially no selection share. Decks that run tricks win no more often than decks without them. Tricks are also cast slightly less often than the average card, because a trick is cast only when a favorable combat arrives. The whole premium is contribution: a trick that gets cast appears overwhelmingly in games its side wins. A pump spell is cast into a combat its caster expects to profit from, so the cast causes the win and coincides with it at once, and the label cannot separate the two.

Sweepers set the selection and contribution channels in direct opposition. Good builders include sweepers, so sweepers sit in decks that win, and their selection share is positive. Their contribution share is negative and larger: a sweeper that gets cast appears disproportionately in games its side loses, because a sweeper is cast when the opponent's board is already ahead, and the side that was ahead usually wins even after the board is cleared. The class nets slightly below average. The corpus's bad builders are not the cause: in games where the forge-best builder plays against itself, a cast sweeper still appears mostly in losses.

The castability bar is near zero for every class, and its sign says nothing about card quality. One cast earns credit if the game is won and blame if it is lost, and the average card's deck wins 46.5% of its games, because the corpus deliberately includes bad builders. One cast is therefore worth slightly less than nothing, and each class's bar multiplies its extra or missing casts by that value: creatures are cast more than average and get a small negative bar, sweepers are cast less and get a small positive one. Morph is the extreme case: its recorded cast rate falls further than any class in the table and its label barely moves. The channel is still worth separating, because with the number of casts removed from the other channels, the contribution bar measures only when a card is cast, not how often.
