...
The training-card fit is also tighter than reading text could ever be. The corpus contains 171 groups of functional reprints: distinct cards whose converted text is identical once the name is stripped, in every line the encoder sees, mana cost, types, and statline included. Balduvian Bears, Bear Cub, Grizzly Bears, and Runeclaw Bear are one group, four names for the same {1}{G} 2/2 vanilla bear; their winnability labels still run from +0.075 down to −0.009, because each name lived in different sets, different decks, and different games. Identical text produces identical embeddings, so any text-based prediction must give such twins one shared value. How much the twins disagree therefore caps what text can explain, at 51–61% of the label variation. The twins only measure that cap; the cards the encoder can memorize are the other 99%, whose stripped text is unique. Its fit on those training cards is above the cap, and its typical training-card error (0.027) is smaller than the twins' typical disagreement (0.029): it fits the part of a label that no reading of the text could predict.
