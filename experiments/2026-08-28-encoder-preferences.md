...
## A card can earn a strong label without ever changing a game

A card gets label credit for its deck's wins, whether or not the card caused them. The winnability label counts the games the card's deck won with the card cast, minus the games it lost with the card cast, over all games the card sat in a deck; whether the deck wins is largely decided by the other 39 cards and by who built it. That mixing is exact and separable: writing w for how often decks containing the card win, m for how often the card gets cast, and d for the difference in cast rate between won and lost games, the label equals m(2w−1) + d/2. The w channel is builder choice: the four Forge build methods win between 26% and 60% of their games, so which builders play a card moves its label by about ±1.8 SD before the card itself does anything. The m channel is castability. The d channel is the only one that measures the card changing games: it is positive when winners cast the card more often than losers did. The decomposition for the headline card classes:
