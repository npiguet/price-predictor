...
The five color-lift labels mostly restate the card's own colors. A color-lift cell is zero by construction whenever that color is one of the card's own, because a card's colors are always present in its own deck. Knowing which cells are zero is therefore the same as knowing the card's colors, which the mana cost gives away (the embedding decodes color at AUC 0.99), and that alone accounts for over half of what these labels ask the encoder to predict.

What the nonzero cells contain is an artifact, not affinity. Their magnitude tracks the card's quality and played rate rather than its pip count, and the label's own arithmetic explains why. Each cell is the difference of two win-rate estimates, and both are pulled toward zero to tame small samples. The with-color estimate rests on fewer games, so it is pulled harder. For a good card the harder-pulled side loses more, and the difference comes out negative in proportion to the card's own quality. A genuine allied-versus-enemy color preference does exist, in both labels and predictions, at about 1.5% of a color-lift SD. A gradient-boosted probe does no better than ridge on these labels, so the shortfall is missing information rather than a linear probe's limit: the cross-color synergy this label family was designed to capture is not in the labels.
