...
The chart gives each card class one bar per channel: the class's deviation on that channel, times what such a deviation is worth to the label at the corpus average. The black dot is the class's exact premium. The bars sum only approximately to the dot; the gap is the share of the premium earned by two channels moving together, which belongs to no single bar, and it stays under 0.13 SD in every row.

Creatures earn about half their premium before the game starts. Creature-heavy decks are the decks that win in this corpus, and every creature in such a deck soaks up credit for the deck's wins whether or not it caused them. Creatures are also cast far more often than spells, but the chart shows that earning them almost nothing; the castability paragraph below explains why. The rest of the premium is game evidence: winning boards are boards with creatures on them. Unconditional removal earns its premium the creature way, mostly by being picked by good builders.

A combat trick ends at nearly the same premium as a creature and earns all of it from timing. Decks that run tricks win no more often than decks that do not, and tricks are cast slightly less often than the average card, sitting in hand until the right combat. The entire premium sits in the contribution channel: when a trick is cast at all, it is overwhelmingly in a game its side wins. A player only fires a pump spell into a fight worth winning, so casting one both causes wins and coincides with them, and the label cannot split the two.

Sweepers are the sharpest contradiction: the builders vouch for them and their appearances incriminate them. Good builders include sweepers, so a sweeper marks a well-built deck and collects selection credit from that deck's wins. But when a sweeper is actually cast, it is disproportionately in a game its side loses: a player casts a sweeper when the board has gotten away from them, and the trouble it answers usually wins anyway. The negative contribution outweighs the selection credit, and the class nets slightly below average. This is not an artifact of the bad builders in the corpus: within the forge-best mirror, casting a sweeper still marks the losing side.

The castability bar is bookkeeping, and reads as a near-zero sliver whose sign flips on attendance alone. An appearance earns credit if the game is won and blame if it is lost, and because the corpus deliberately includes bad builders, the average card's deck wins only 46.5% of its games. One appearance is therefore worth slightly less than nothing, and the bar multiplies the class's extra or missing appearances by that near-zero value: creatures, cast more than average, get a small negative sliver, and sweepers, cast less, a small positive one. The extreme case is morph, whose recorded cast rate collapses further than any class in the table while its label barely moves: in this accounting, not showing up is nearly free. The channel still earns its place in the split, because with the quantity of appearances factored out, the contribution bar is purely about their timing.
