...
Part of every label is unpredictable from text, and the functional reprints measure how much. The corpus contains 171 groups of them: distinct cards whose converted text is identical once the name is stripped, in every line the encoder sees, mana cost, types, and statline included. Balduvian Bears, Bear Cub, Grizzly Bears, and Runeclaw Bear are one group, four names for the same {1}{G} 2/2 vanilla bear; their winnability labels still run from +0.075 down to −0.009, because each name lived in different sets, different decks, and different games. Identical text forces identical embeddings, so the twins get one shared prediction, and their disagreement measures the part of a label that text cannot carry. For a card whose games the model never saw, that part cannot be predicted, only estimated in size. Taking the twins' spread as the estimate, at most 51–61% of the label variation is predictable from text, and the encoder's 37% on unseen cards sits under that ceiling. The estimate is an extrapolation from the twins to the rest of the corpus, and it leans conservative: the twins are among the best-measured cards in the corpus, with several times a typical card's games behind their labels, so a typical card's circumstance part is, if anything, larger and its ceiling lower.

On its training cards the encoder beats that ceiling, which only stored answers can do. The ceiling binds prediction, not recall: 99% of stripped texts are unique in the corpus, so a card's exact wording can serve as a name, and training pushes each unique wording's prediction toward that card's own measured label until the value is simply stored. The text supplies only the key; the stored value arrives through the other half of training, the measured label itself, game circumstances included. The margin is the tell: the typical training-card error (0.027) is smaller than the twins' typical disagreement (0.029), so the encoder reproduces even the part of its training labels it could never predict for a new card. A twin-free measurement agrees on the floor's size: splitting every card's games in half and comparing the two halves puts the sampling-noise part of a label alone at SD 0.025, before any circumstance differences, and the training error sits essentially on top of it. The twins show the flip side: they cannot be told apart, so a card without unique text cannot be memorized.

![The encoder's fit on training cards, on shuffled training cards, and on unseen cards, against the unseen-card text ceiling](images/2026-08-28-encoder-memorization.png)
