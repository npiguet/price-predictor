...
def _load_dataset(
    ...
    _log(
        ...
        f"(loaded card embeddings + parsed text for deck stats, "
...
def _batch_to_device(batch: TrainingBatch, device: torch.device) -> TrainingBatch:
    return TrainingBatch(
        ...
        winner_deck_stats=batch.winner_deck_stats.to(device),
        loser_deck_stats=batch.loser_deck_stats.to(device),
    )
...
def _score_batch(
    ...
    return (
        model(winner_cards, batch.winner_mask, batch.winner_deck_stats),
        model(loser_cards, batch.loser_mask, batch.loser_deck_stats),
    )
